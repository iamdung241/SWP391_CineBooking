...
public class SearchAccountStaff extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        if(searchQuery.isBlank() && roleFilter.isBlank() && theaterFilter.isBlank()){
            response.sendRedirect("views/dashboard/managestaff.jsp");
        }
        else {
            request.setAttribute("accounts", accounts);
            request.getRequestDispatcher("views/dashboard/managestaff.jsp").forward(request, response);
        }
    } 
}
